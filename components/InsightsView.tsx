...
export const InsightsView: React.FC = () => {
  ...
  const fetchAnalysis = async () => {
    ...
    try {
      ...
      setError("Failed to generate AI insights. Please ensure your API key is configured.");
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="p-8 text-center bg-white rounded-xl border border-red-100 shadow-sm">
        ...
        <button 
          ...
          className="px-6 py-2 bg-slate-900 text-white rounded-lg text-sm font-medium hover:bg-slate-800 transition-all"
        >
          Try Again
        </button>
      </div>
        ...
        <div className="h-[400px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={combinedChartData} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
              ...
              />
              {/* Fix: Recharts Area component strokeDasharray prop expects a string or number, not a function. */}
              <Area 
                type="monotone" 
                dataKey="expenses" 
                stroke="#22c55e" 
